    ...
    <v-row>
      <v-col cols="12" md="4" class="">
        <v-form class="px-10">
          ...
          <CustomerSelect ref="customer" class="my-2" @customer-selected="setCustomer" />
          <AmountInput ref="amount" class="my-2" />
          <v-select :items="['fastest', 'standard']" v-model="settlement_speed" label="Settlement Speed"></v-select>
          <v-select :items="['automatic', 'instant', 'microdeposits']" v-model="verification_method" label="Verification Method"></v-select>
        ...
        <v-card>
          ...
          <v-card-text>
            ...
            <v-alert v-else type="info" border="left">
              The ACH UI will be loaded once the Intent is created
            </v-alert>
          </v-card-text>
        </v-card>
      </v-col>
...
<script>
import CardHeader from '~/components/core/CardHeader.vue';
...
export default {
    ...
    methods: {
      setCustomer(e){
        // console.log("SEt Customer called", e);
        this.customer = e
        this.account.name = this.customer.name
        this.account.email = this.customer.email
      },
      getParams() {
        const params = {
          ...
          payment_method_options: {
            us_bank_account: {
              ...
              preferred_settlement_speed: this.settlement_speed
            }
          }
        }
        if (this.sfu) {
      ...
      async createPaymentIntent() {
        ...
        const {intent} = await this.$axios.$post('payment_intents/', params)
      ...
      async getStatus() {
          const {paymentIntent} = await this.$stripe.retrievePaymentIntent(this.paymentIntent.client_secret);
          this.paymentIntent = paymentIntent
      }
    }
}
